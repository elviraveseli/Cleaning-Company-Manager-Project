...
export class InvoiceService {
  ...
  sendInvoiceEmail(invoiceId: string, emailData: any): Observable<{ message: string }> {
    ...
    return this.exportInvoicePDF(invoiceId).pipe(
      switchMap(pdfBlob => {
      ...
      switchMap(pdfBase64 => {
        // Create email data with PDF attachment and payment link
        const emailPayload = {
          ...emailData,
          attachments: [{
            filename: `invoice-${invoiceId}.pdf`,
            content: pdfBase64.split(',')[1], // Remove data:application/pdf;base64,
            encoding: 'base64',
            contentType: 'application/pdf'
          }],
          includePaymentLink: true // Backend will generate a unique payment link
        };

        // Send email and mark as sent
        return this.http.post<{ message: string }>(`${this.apiUrl}/${invoiceId}/send-email`, emailPayload, {
          headers: this.getHttpHeaders()
        });
      }),
  ...
  // Track email status
  getEmailStatus(invoiceId: string): Observable<{ sent: boolean, openedAt?: Date, clickedAt?: Date }> {
    return this.http.get<{ sent: boolean, openedAt?: Date, clickedAt?: Date }>(
      `${this.apiUrl}/${invoiceId}/email-status`,
      { headers: this.getHttpHeaders() }
    ).pipe(
      catchError(this.handleError<{ sent: boolean, openedAt?: Date, clickedAt?: Date }>('getEmailStatus'))
    );
  }

  // Verify payment token from email link
  verifyPaymentToken(invoiceId: string, token: string): Observable<{ valid: boolean, invoice?: Invoice }> {
    return this.http.post<{ valid: boolean, invoice?: Invoice }>(
      `${this.apiUrl}/${invoiceId}/verify-payment`,
      { token },
      { headers: this.getHttpHeaders() }
    ).pipe(
      catchError(this.handleError<{ valid: boolean, invoice?: Invoice }>('verifyPaymentToken'))
    );
  }
  ...
  updatePayment(invoiceId: string): Observable<Invoice> {
  ...
  markAsSent(invoiceId: string): Observable<Invoice> {
    return this.http.post<Invoice>(`${this.apiUrl}/${invoiceId}/mark-sent`, {}, {
      headers: this.getHttpHeaders()
    }).pipe(
