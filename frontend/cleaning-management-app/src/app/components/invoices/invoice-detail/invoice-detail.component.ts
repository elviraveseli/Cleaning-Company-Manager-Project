...
export class InvoiceDetailComponent implements OnInit, OnDestroy {
  ...
  initializeForm() {
    ...
    // Add initial service if not editing
    if (!this.isEdit) {
      this.addService();
    }
      ...
      .subscribe({
        next: (contracts) => {
          ...
          if (contracts.length > 0) {
            console.log('Sample contract structure:', contracts[0]);
          }
        },
        error: (error) => {
    ...
    this.objectService
      ...
      .subscribe({
        next: (objects) => {
          ...
          // Log structure of first object for debugging
          if (objects.length > 0) {
            console.log('Sample object structure:', objects[0]);
          }
          this.loading = false;
        },
        error: (error) => {
  ...
  populateFromContract(contract: any) {
    if (contract) {
      ...
      console.log('Billing address from contract:', contract.billingAddress);
      console.log(
        'Customer address from contract:',
      ...
      if (contract.billingAddress) {
        ...
        if (contract.billingAddress.sameAsService) {
          ...
          console.log('Using customer address:', billingAddress);
        } else {
          // Use billing address
          billingAddress = {
        ...
        console.log(
          'No billing address defined, using customer address:',
          billingAddress
        );
      }

      console.log('Final billing address to be used:', billingAddress);
      ...
      if (contract.services && contract.services.length > 0) {
        ...
        servicesArray.push(
          this.createServiceFormGroup({
            description: `Cleaning services for ${contract['contractNumber']}`,
            quantity: 1,
            unitPrice: contract['totalAmount'] || 0,
            total: contract['totalAmount'] || 0,
          })
        );
      }
      this.calculateTotals();
  ...
  calculateTotals() {
    ...
    this.totalAmount =
      Math.round((this.subtotal + this.taxAmount - discount) * 100) / 100;
  }
  ...
  getObjectDisplayName(object: any): string {
    ...
    const name =
      object.name || object.objectName || object.title || 'Unnamed Object';
    const address =
      object.address ||
      object.location ||
      object.street ||
  ...
  onSubmit() {
    ...
    // Add customer ID if we have a selected customer
    if (this.selectedCustomer && this.selectedCustomer._id) {
      customerData.customerId = this.selectedCustomer._id;
    }
    ...
    const servicesData = this.servicesArray.controls.map((control) => {
      ...
      return {
        ...
        total:
          service.total || (service.quantity || 1) * (service.unitPrice || 0),
    ...
    const invoiceData: any = {
      ...
      customerContract: this.selectedContract?._id || null, // Send only the ID
